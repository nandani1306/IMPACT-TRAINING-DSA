...
void addEdge(Graph* graph, int u, int v, int wt){
    if(!isVertex(graph,u) || !isVertex(graph,v)){
        ...
        graph->adjList[graph->totalEdges-1] = (int*)malloc(sizeof(int)*3);
        graph->adjList[graph->totalEdges-1][0] = u;
        graph->adjList[graph->totalEdges-1][1] = v;
        graph->adjList[graph->totalEdges-1][2] = wt;
    }
}
    ...
    if(!isVertex(graph,u) || !isVertex(graph,v)){
        ...
        for (int i = 0; i < graph->totalEdges; i++)
        {
            if(graph->adjList[i][0] == u && graph->adjList[i][1] == v){
                ...
                graph->adjList[i][2] = graph->adjList[graph->totalEdges-1][2];
                graph->adjList = realloc(graph->adjList, sizeof(int*)*(--graph->totalEdges));
            }
        }   
    }
...
void printGraph(Graph* graph){
    if(!graph){
        ...
        for(int i = 0; i < graph->totalEdges; i++){
            printf("[%d, %d, %d]\n",graph->adjList[i][0], graph->adjList[i][1], graph->adjList[i][2]);
        }
        printf("\n");
    }
}
